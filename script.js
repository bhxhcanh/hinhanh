...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    /**
     * Tải và giải mã một ảnh từ một nguồn (URL dữ liệu) và trả về một Promise.
     * Hàm này khắc phục race condition bằng cách sử dụng image.decode() để đảm bảo
     * ảnh được tải và giải mã hoàn toàn trước khi được sử dụng.
    ...
    const loadImage = (src) => {
        return new Promise(async (resolve, reject) => {
            try {
                const img = new Image();
                img.src = src;
                // Chờ cho ảnh được tải xuống và giải mã hoàn toàn.
                // Đây là bước quan trọng để tránh lỗi race condition.
                await img.decode();
                
                // Kiểm tra các ảnh bị hỏng có kích thước bằng không.
                if (img.naturalWidth === 0 || img.naturalHeight === 0) {
                    reject(new Error('Tệp ảnh bị hỏng hoặc có kích thước bằng không.'));
                    return;
                }
                resolve(img);
            } catch (error) {
                console.error("Lỗi tải hoặc giải mã ảnh:", error);
                reject(new Error('Không thể tải hoặc giải mã ảnh. Tệp có thể không được hỗ trợ hoặc bị hỏng.', { cause: error }));
            }
        });
    };
